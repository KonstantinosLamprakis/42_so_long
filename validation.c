...
/*   Updated: 2024/04/10 13:47:37 by klamprak         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**get_map(int argc, char **argv)
{
	...
	if (!check_args(argc, argv))
		return (clean_map_file(NULL, -1, "Wrong Args\n"));
	fd = open(argv[1], O_RDONLY);
	if (fd == -1)
		return (NULL);
	if (!check_nl(&i, fd) || i < 3)
		return (clean_map_file(NULL, fd, "Remove new lines\n"));
	if (close(fd) == -1)
		return (NULL);
	fd = open(argv[1], O_RDONLY);
	if (fd == -1)
...
static char	**validate_map(char **map, int fd)
{
	if (!check_borders(map))
		return (clean_map_file(map, fd, "Check walls\n"));
	if (!check_rectagle(map))
		return (clean_map_file(map, fd, "Should be rectagle\n"));
	if (!check_chars(map))
		return (clean_map_file(map, fd, "Check characters\n"));
	if (close(fd) == -1)
